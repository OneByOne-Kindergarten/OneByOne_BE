...
public interface CommunityLikeRepository extends JpaRepository<CommunityLike, Long> {
  ...
  @Query(
      "SELECT new com.onebyone.kindergarten.domain.communityPosts.dto.response.CommunityLikeResponseDTO("
          ...
          + "WHERE cp.id = :postId "
          + "GROUP BY cp")
  Optional<CommunityLikeResponseDTO> findLikeInfo(
      @Param("postId") Long postId, @Param("user") User user);

  @Query("SELECT cl FROM CommunityLike cl " + "WHERE cl.user = :user AND cl.post.id = :postId")
  Optional<CommunityLike> findByUserAndPostId(
      @Param("user") User user, @Param("postId") Long postId);
