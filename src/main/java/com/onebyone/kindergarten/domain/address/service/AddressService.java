...
public class AddressService {
  ...
  public List<AddressResponseDTO> getAddress() {
    ...
    List<SubRegionDTO> subRegions =
        subRegionRepository.findAll().stream().map(SubRegion::toDTO).toList();

    // 시군구 map 생성
    Map<Long, SubRegionResponseDTO> subRegionMap =
        subRegions.stream()
            .collect(
                Collectors.toMap(
    ...
    return regions.stream()
        .map(
            r -> {
              ...
              List<SubRegionResponseDTO> regionSubRegions =
                  rootSubRegions.stream()
                      .filter(
                          s ->
                              subRegions.stream()
                                  .anyMatch(
                                      sr ->
                                          sr.getSubRegionId().equals(s.getSubRegionId())
