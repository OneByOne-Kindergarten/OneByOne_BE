...
public class PushNotificationController {
    ...
    public ResponseDto<String> sendNotification(@RequestBody PushNotificationRequestDTO requestDTO) {
    ...
    public ResponseDto<List<PushNotificationResponseDTO>> getUserNotifications(@PathVariable Long userId) {
    ...
    public ResponseDto<List<PushNotificationResponseDTO>> getMyNotifications(@AuthenticationPrincipal UserDetails userDetails) {
        List<PushNotificationResponseDTO> notifications = pushNotificationService.getUserNotificationByUserDetails(userDetails);
        return ResponseDto.success(notifications);
    }

    @GetMapping("/my/unread")
    @Operation(summary = "현재 로그인한 사용자의 읽지 않은 알림 조회", description = "현재 로그인한 사용자의 읽지 않은 알림을 조회합니다.")
    public ResponseDto<List<PushNotificationResponseDTO>> getMyUnreadNotifications(@AuthenticationPrincipal UserDetails userDetails) {
        List<PushNotificationResponseDTO> notifications = pushNotificationService.getUnreadNotificationsByUserDetails(userDetails);
        return ResponseDto.success(notifications);
    }

    @GetMapping("/my/unread/count")
    @Operation(summary = "현재 로그인한 사용자의 읽지 않은 알림 개수 조회", description = "현재 로그인한 사용자의 읽지 않은 알림 개수를 조회합니다.")
    public ResponseDto<Long> getUnreadCount(@AuthenticationPrincipal UserDetails userDetails) {
        long count = pushNotificationService.countUnreadNotifications(userDetails);
    ...
    public ResponseDto<String> markAllAsRead(@AuthenticationPrincipal UserDetails userDetails) {
        pushNotificationService.markAllAsRead(userDetails);
