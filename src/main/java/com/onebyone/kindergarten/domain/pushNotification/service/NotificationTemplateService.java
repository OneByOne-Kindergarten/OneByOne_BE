...
/** 알림 타입별 템플릿을 관리하고 발행하는 서비스 모든 알림 발송 로직은 이 클래스를 통해 처리하여 일관성 있는 알림 메시지 형식 유지 */
@Slf4j
@Service
...
public class NotificationTemplateService {
  ...
  private static final String GROUP_LIKE_MESSAGE_2 =
      "%s %s님과 1명이 회원님의 게시글을 좋아합니다\n👀 지금 바로 확인해보세요!";
  private static final String GROUP_LIKE_MESSAGE_N =
      "%s %s님 외 %d명이 회원님의 게시글을 좋아합니다\n👀 지금 바로 확인해보세요!";
  ...
  public void sendCommentNotification(
      Long targetUserId, User actionUser, String content, boolean isReply, Long postId) {
    if (!targetUserId.equals(actionUser.getId())) {
      ...
      Optional<PushNotification> existingNotification =
          findGroupableNotification(targetUserId, groupKey, sinceTime);
  ...
  public void sendLikeNotification(
      Long targetUserId, User actionUser, String contentTitle, Long targetId) {
    if (!targetUserId.equals(actionUser.getId())) {
      ...
      if (existingNotification.isPresent()) {
        ...
        createNewLikeNotification(
            targetUserId, actionUser, targetId, groupKey, NotificationType.LIKE);
      }
    }
  }
  ...
  /** 리뷰 좋아요 알림을 발송합니다. */
  @Transactional
  public void sendReviewLikeNotification(
      Long targetUserId, User actionUser, String contentTitle, Long targetId) {
    if (!targetUserId.equals(actionUser.getId())) {
      ...
      if (existingNotification.isPresent()) {
        ...
        createNewLikeNotification(
            targetUserId, actionUser, targetId, groupKey, NotificationType.REVIEW);
      }
    }
  }

  /** 문의 답변 알림을 발송합니다. */
  public void sendInquiryAnswerNotification(
      Long targetUserId, String inquiryTitle, Long inquiryId) {
    String message = "✅ 문의하신 내용에 답변이 등록되었습니다." + CHECK_APP_MESSAGE;
    PushNotificationEvent event =
        createNotificationEvent(
            targetUserId, "문의 답변 알림", message, NotificationType.SYSTEM, inquiryId, null);
  ...
  public void sendNoticeNotification(
      Long targetUserId, String noticeTitle, String noticeContent, Long noticeId) {
    String message = "📢 [" + noticeTitle + "]\n" + CHECK_APP_MESSAGE;
    PushNotificationEvent event =
        createNotificationEvent(
            targetUserId, "공지사항", message, NotificationType.NOTICE, noticeId, null);
    notificationEventPublisher.publish(event);
  ...
  public void sendSystemNotification(
      Long targetUserId, String title, String content, Long targetId) {
    String message = "🔔 [" + title + "]\n" + CHECK_APP_MESSAGE;
    PushNotificationEvent event =
        createNotificationEvent(
            targetUserId, "시스템 알림", message, NotificationType.SYSTEM, targetId, null);
    notificationEventPublisher.publish(event);
    log.debug("시스템 알림 발송: 대상자={}", targetUserId);
  }

  /** 모든 활성 사용자에게 공지사항 알림을 발송합니다. 사용자별 알림 설정을 확인하여 알림을 받을 사용자에게만 전송합니다. 배치 처리로 스레드 풀 포화를 방지합니다. */
  public void sendNoticeNotificationToAllUsers(
      String noticeTitle, String noticeContent, Long noticeId) {
    try {
      List<User> activeUsers = userRepository.findAllActiveUsers();

      log.info("공지사항 푸시 알림 전송 시작 - 공지 ID: {}, 전체 사용자 수: {}", noticeId, activeUsers.size());
      ...
      for (int i = 0; i < activeUsers.size(); i += BATCH_SIZE) {
        ...
        log.debug("배치 처리 중: {}/{} (배치 크기: {})", endIndex, activeUsers.size(), batch.size());

        /// 푸시 알림 전송
        for (User user : batch) {
          try {
            if (shouldSendNoticeNotification(user)) {
              sendNoticeNotification(user.getId(), noticeTitle, noticeContent, noticeId);
              sentCount++;
            } else {
              skippedCount++;
            ...
            log.error("사용자 {}에게 공지사항 푸시 알림 전송 실패: {}", user.getId(), e.getMessage(), e);
            skippedCount++;
          }
        }
      ...
      log.info("공지사항 푸시 알림 전송 완료 - 공지 ID: {}, 전송: {}명, 스킵: {}명", noticeId, sentCount, skippedCount);
    } catch (Exception e) {
      log.error("공지사항 푸시 알림 전송 중 오류 발생 - 공지 ID: {}, 오류: {}", noticeId, e.getMessage(), e);
    }
  }
  ...
  /** 알림 그룹화 가능 여부를 확인합니다. 미전송 알림 중에서만 그룹화 대상을 찾습니다. */
  private Optional<PushNotification> findGroupableNotification(
      Long userId, String groupKey, LocalDateTime sinceTime) {
    return notificationRepository
        .findFirstByUserIdAndGroupKeyAndIsSentFalseOrderByCreatedAtDesc(userId, groupKey)
        .filter(notification -> notification.getCreatedAt().isAfter(sinceTime));
  }

  /** 알림 그룹화 처리 */
  private void handleGroupedNotification(
      PushNotification notification, User actionUser, boolean isReply) {
    notification.increaseGroupCount();
    String updatedMessage =
        createGroupedMessage(actionUser.getNickname(), notification.getGroupCount(), isReply);
  ...
  private void handleGroupedLikeNotification(PushNotification notification, User actionUser) {
    notification.increaseGroupCount();
    String updatedMessage =
        createGroupedLikeMessage(actionUser.getNickname(), notification.getGroupCount());
    notification.updateGroupMessage(updatedMessage);
    notificationRepository.save(notification);
    log.debug(
        "좋아요 알림 그룹화 (count={}): 대상자={}",
        notification.getGroupCount(),
        notification.getUser().getId());
  }

  /** 새로운 댓글 알림 생성 */
  private void createNewCommentNotification(
      Long targetUserId, User actionUser, boolean isReply, Long postId, String groupKey) {
    ...
    PushNotificationEvent event =
        createNotificationEvent(
            targetUserId, title, message, NotificationType.COMMENT, postId, groupKey);
  ...
  /** 새로운 좋아요 알림 생성 */
  private void createNewLikeNotification(
      Long targetUserId, User actionUser, Long targetId, String groupKey, NotificationType type) {
    String message = String.format(SINGLE_LIKE_MESSAGE, LIKE_EMOJI, actionUser.getNickname());

    PushNotificationEvent event =
        createNotificationEvent(targetUserId, "좋아요 알림", message, type, targetId, groupKey);
  ...
  private String createGroupedMessage(String actorName, int count, boolean isReply) {
    ...
    if (count == 2) {
      return String.format(
          isReply ? GROUP_REPLY_MESSAGE_2 : GROUP_COMMENT_MESSAGE_2, emoji, actorName);
    }
    return String.format(
        isReply ? GROUP_REPLY_MESSAGE_N : GROUP_COMMENT_MESSAGE_N, emoji, actorName, count - 1);
  }
