...
public class KindergartenWorkReviewService {
  ...
  public Kindergarten createWorkReview(CreateWorkReviewRequestDTO request, Long userId) {
    ...
    KindergartenWorkReview review =
        KindergartenWorkReview.builder()
            .user(user)
            .kindergarten(kindergarten)
  ...
  public Kindergarten modifyWorkReview(ModifyWorkReviewRequestDTO request, Long userId) {
    ...
    KindergartenWorkReview review =
        workReviewRepository
            .findById(request.getWorkReviewId())
  ...
  public void likeWorkReview(long reviewId, Long userId) {
    ...
    Optional<KindergartenWorkReviewLikeHistory> existingLike =
        workReviewLikeHistoryRepository.findByUserAndWorkReview(user, review);
    ...
    if (existingLike.isPresent()) {
      ...
      // 좋아요 추가
      KindergartenWorkReviewLikeHistory newLike =
          KindergartenWorkReviewLikeHistory.builder().user(user).workReview(review).build();
  ...
  public WorkReviewPagedResponseDTO getReviews(
      ...
      int page,
      int size,
      WorkReviewPagedResponseDTO.SortType sortType,
      WorkReviewStarRatingType workReviewStarRatingType,
      int starRating) {
    if (workReviewStarRatingType != WorkReviewStarRatingType.ALL && starRating < 1
        || starRating > 5) {
    ...
    switch (workReviewStarRatingType) {
      case BENEFIT_AND_SALARY:
        reviewPage =
            kindergartenWorkReviewRepository.findByBenefitAndSalaryScore(
                kindergartenId, ReviewStatus.ACCEPTED, starRating, pageable);
        break;
      case WORK_LIFE_BALANCE:
        reviewPage =
            kindergartenWorkReviewRepository.findByWorkLifeBalanceScore(
                kindergartenId, ReviewStatus.ACCEPTED, starRating, pageable);
        break;
      case WORK_ENVIRONMENT:
        reviewPage =
            kindergartenWorkReviewRepository.findByWorkEnvironmentScore(
                kindergartenId, ReviewStatus.ACCEPTED, starRating, pageable);
        break;
      case MANAGER:
        reviewPage =
            kindergartenWorkReviewRepository.findByManagerScore(
                kindergartenId, ReviewStatus.ACCEPTED, starRating, pageable);
        break;
      case CUSTOMER:
        reviewPage =
            kindergartenWorkReviewRepository.findByCustomerScore(
                kindergartenId, ReviewStatus.ACCEPTED, starRating, pageable);
        break;
      default:
        reviewPage =
            workReviewRepository.findReviewsWithUserInfo(
  ...
  public WorkReviewPagedResponseDTO getMyReviews(Long userId, int page, int size) {
    ...
    Page<WorkReviewDTO> reviewPage =
        workReviewRepository.findMyReviews(userId, ReviewStatus.ACCEPTED, pageable);
  ...
  public WorkReviewPagedResponseDTO getAllReviews(
      int page, int size, WorkReviewPagedResponseDTO.SortType sortType) {
    Pageable pageable;
    ...
    Page<WorkReviewDTO> reviewPage =
        workReviewRepository.findAllReviewsWithUserInfo(ReviewStatus.ACCEPTED, pageable);
