...
@Repository
public interface UserFavoriteKindergartenRepository
    extends JpaRepository<UserFavoriteKindergarten, Long> {

  @Query(
      "SELECT new com.onebyone.kindergarten.domain.kindergatens.dto.KindergartenResponseDTO("
  ...
  @Modifying
  @Query(
      "DELETE FROM user_favorite_kindergarten uf "
          + "WHERE uf.user = :user AND uf.kindergarten.id = :kindergartenId")
  void deleteByUserAndKindergartenId(
      @Param("user") User user, @Param("kindergartenId") Long kindergartenId);

  @Query(
      "SELECT COUNT(uf) > 0 FROM user_favorite_kindergarten uf "
          + "WHERE uf.user = :user AND uf.kindergarten.id = :kindergartenId")
  boolean existsByUserAndKindergartenId(
      @Param("user") User user, @Param("kindergartenId") Long kindergartenId);
  ...
  @EntityGraph(
      attributePaths = {
        "kindergarten",
        "kindergarten.kindergartenInternshipReviewAggregate",
