...
public class ReportService {
  ...
  public Page<ReportResponseDTO> getAllReports(ReportSearchDTO searchDTO, Pageable pageable) {
    ...
    return reportRepository.findAllDtosByCondition(
        searchDTO.getStatus(), searchDTO.getTargetType(), pageable);
  }
  ...
  /// 신고 대상 상태 업데이트
  private void _updateTargetStatus(
    ...
    switch (targetType) {
      ...
      case WORK_REVIEW -> {
        KindergartenWorkReview kindergartenWorkReview =
            kindergartenWorkReviewRepository
                .findById(targetId)
                .orElseThrow(
                    () -> new BusinessException(ErrorCodes.INVALID_REPORT_WORK_REVIEW_TARGET));
        kindergartenWorkReview.updateStatus(status);
      }
      case INTERNSHIP_REVIEW -> {
        KindergartenInternshipReview kindergartenInternshipReview =
            kindergartenInternshipReviewRepository
                .findById(targetId)
                .orElseThrow(
                    () ->
                        new BusinessException(ErrorCodes.INVALID_REPORT_INTERNSHIP_REVIEW_TARGET));
