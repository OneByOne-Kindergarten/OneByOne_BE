...
public class KindergartenInternshipReviewAggregateService {
  ...
  public void updateOrCreateAggregate(Kindergarten kindergarten) {
    List<KindergartenInternshipReview> acceptedReviews =
        kindergartenInternshipReviewRepository.findByKindergartenAndReviewStatus(
            kindergarten, ReviewStatus.ACCEPTED);
    ...
    BigDecimal avgWorkEnvironmentScore =
        BigDecimal.valueOf((double) totalWorkEnvironmentScore / reviewCount);
    BigDecimal avgLearningSupportScore =
        BigDecimal.valueOf((double) totalLearningSupportScore / reviewCount);
    BigDecimal avgInstructionTeacherScore =
        BigDecimal.valueOf((double) totalInstructionTeacherScore / reviewCount);

    KindergartenInternshipReviewAggregate aggregate =
        kindergartenInternshipReviewAggregateRepository.findByKindergarten(kindergarten);

    aggregate.updateScoreAggregates(
        avgWorkEnvironmentScore, avgLearningSupportScore, avgInstructionTeacherScore);
  }
}
