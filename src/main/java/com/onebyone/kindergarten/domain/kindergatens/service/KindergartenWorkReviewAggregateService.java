...
public class KindergartenWorkReviewAggregateService {
  ...
  public void updateOrCreateAggregate(Kindergarten kindergarten) {
    List<KindergartenWorkReview> acceptedReviews =
        workReviewRepository.findByKindergartenAndReviewStatus(kindergarten, ReviewStatus.ACCEPTED);
    ...
    BigDecimal avgBenefitAndSalary =
        BigDecimal.valueOf((double) totalBenefitAndSalary / reviewCount);
    ...
    KindergartenWorkReviewAggregate aggregate =
        workReviewAggregateRepository.findByKindergarten(kindergarten);
    aggregate.updateScoreAggregates(
        avgBenefitAndSalary, avgWorkLifeBalance, avgWorkEnvironment, avgManager, avgCustomer);
  }
}
