...
public class KindergartenController {
  ...
  public ResponseEntity<Boolean> saveKindergartens(
      @RequestBody List<KindergartenDTO> kindergartenDTOs) {
  ...
  public ResponseEntity<PageResponseDTO<KindergartenResponseDTO>> searchKindergartens(
      KindergartenSearchDTO searchDTO, @PageableDefault(size = 10) Pageable pageable) {
    Page<KindergartenResponseDTO> page =
        kindergartenService.searchKindergartens(searchDTO, pageable);
    return ResponseEntity.ok(new PageResponseDTO<>(page));
  }

  @GetMapping("/{id}")
  @Operation(summary = "유치원 상세 조회", description = "유치원의 ID를 사용하여 유치원 정보를 조회합니다.")
  ...
  @GetMapping("/nearby")
  @Operation(summary = "주변 유치원 조회", description = "현재 위치 기준으로 특정 반경 내의 유치원을 조회합니다.")
  public ResponseDto<List<KindergartenResponseDTO>> getNearby(
      @RequestParam double latitude,
      @RequestParam double longitude,
      @RequestParam(defaultValue = "2.0") double radiusKm) {
    return ResponseDto.success(
        kindergartenService.getNearbyKindergarten(latitude, longitude, radiusKm));
  }
