...
public interface KindergartenRepository extends JpaRepository<Kindergarten, Long> {
  ...
  @EntityGraph(
      attributePaths = {"kindergartenInternshipReviewAggregate", "kindergartenWorkReviewAggregate"})
  @Query(
  ...
  Page<Kindergarten> findBySearchCriteria(
      @Param("search") KindergartenSearchDTO search, Pageable pageable);
  ...
  @EntityGraph(
      attributePaths = {"kindergartenInternshipReviewAggregate", "kindergartenWorkReviewAggregate"})
  Optional<Kindergarten> findWithReviewsById(Long id);

  /// 유치원 이름과 주소로 검색
  @Query("SELECT k FROM Kindergarten k WHERE k.name = :name AND k.address = :address")
  Optional<Kindergarten> findByNameAndAddress(
  ...
  List<Kindergarten> findNearbyKindergartenEntities(
