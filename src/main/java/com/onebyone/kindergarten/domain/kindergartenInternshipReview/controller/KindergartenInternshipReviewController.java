...
public class KindergartenInternshipReviewController {
  ...
  public void likeInternshipReview(
      ...
      @AuthenticationPrincipal UserDetails userDetails) {
    kindergartenInternshipReviewService.likeInternshipReview(
        id, Long.valueOf(userDetails.getUsername()));
  }
  ...
  public InternshipReviewPagedResponseDTO getReviews(
    ...
    return kindergartenInternshipReviewService.getReviews(
        kindergartenId, page, size, sortType, internshipReviewStarRatingType, starRating);
  }

  @Operation(
      summary = "실습리뷰-05 리뷰 삭제",
      description = "실습 리뷰를 삭제합니다. 본인이 작성한 리뷰 또는 관리자가 삭제할 수 있습니다.")
  ...
  @Operation(
      summary = "실습리뷰-06 전체 리뷰 조회",
      description = "유치원 상관없이 전체 실습 리뷰를 페이징 조회합니다. (정렬: LATEST-최신순, POPULAR-인기순)")
  @GetMapping("/reviews")
