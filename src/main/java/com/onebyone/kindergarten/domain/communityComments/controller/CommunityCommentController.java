...
public class CommunityCommentController {
  ...
  @Operation(
      summary = "댓글 작성",
      description = "게시글에 댓글이나 대댓글을 작성합니다. 대댓글인 경우 parentId에 원댓글 ID를 입력합니다.")
  public ResponseDto<CommentResponseDTO> createComment(
      @PathVariable Long postId,
      @Valid @RequestBody CreateCommentRequestDTO dto,
      @AuthenticationPrincipal UserDetails userDetails) {
    return ResponseDto.success(
        commentService.createComment(postId, dto, Long.valueOf(userDetails.getUsername())));
  }
  ...
  public PageResponseDTO<CommentResponseDTO> getAllComments(
    ...
    return new PageResponseDTO<>(
        commentService.getAllCommentsWithReplies(postId, pageable, userId));
  }
}
