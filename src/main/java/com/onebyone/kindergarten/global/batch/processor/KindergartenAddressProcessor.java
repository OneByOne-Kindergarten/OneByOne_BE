...
public class KindergartenAddressProcessor implements ItemProcessor<Kindergarten, Kindergarten> {
    ...
    public Kindergarten process(Kindergarten item){
        ...
        if(cityId == null){
            if("전북특별자치도".equals(city)) {
                ...
                System.out.println("등록되지 않은 city: " + item.getId() + " / " + city + " / " + item.getAddress());
                return item;
            }
        }
        ...
        if(subMap == null){
            System.out.println("서브리전 없음: " + item.getId() + " / cityId=" + cityId + " / " + item.getAddress());
            return item;
        }
        ...
        if(subRegionEntity == null){
            System.out.println("해당 subRegion 없음: " + item.getId() + " / " + subRegion + " / " + item.getAddress());
            return item;
        }
    ...
    public static KindergartenAddressProcessor create(RegionRepository regionRepository, SubRegionRepository subRegionRepository) {
        ...
        Map<Long, Map<String, SubRegion>> subRegionMap = subRegionRepository.findAll()
                .stream()
                .collect(Collectors.groupingBy(
                        SubRegion::getRegionId,
